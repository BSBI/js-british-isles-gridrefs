 ...
 * @constructor
 */
var GridRefParser$1 = function GridRefParser() {};

/**
 * x,y offsets (in metres) for tetrad letter codes
 * @type {Object.<string,Array.<number>>}
 */
GridRefParser$1.tetradOffsets = {
 ...
 * @var array
 */
GridRefParser$1.quadrantOffsets = {
  NW: [0, 5000],
  NE: [5000, 5000],
  SW: [0, 0],
  SE: [5000, 0]
 ...
 * @var string
 */
GridRefParser$1.tetradLetters = 'ABCDEFGHIJKLMNPQRSTUVWXYZ';

/**
 *
 * @var string
 */
GridRefParser$1.prototype.preciseGridRef = '';
 ...
 * @var number
 */
GridRefParser$1.prototype.length = 0;

/**
 * @var string
 */
GridRefParser$1.prototype.hectad = '';
 ...
 * @var string
 */
GridRefParser$1.prototype.tetrad = '';

/**
 *
 * @var string
 */
GridRefParser$1.prototype.tetradLetter = '';
 ...
 * @var string
 */
GridRefParser$1.prototype.quadrant = '';
 ...
 * @var string
 */
GridRefParser$1.prototype.quadrantCode = '';

/**
 * update tetrad using Easting/Northing values (metres)
 * hectad should have been set prior to call
 */
GridRefParser$1.prototype.set_tetrad = function () {
  this.tetradLetter = GridRefParser$1.tetradLetters.substr((Math.floor(this.gridCoords.x % 10000 / 1000) >> 1) * 5 + (Math.floor(this.gridCoords.y % 10000 / 1000) >> 1), 1);
...
GridRefParser$1.get_normalized_precision = function (rawPrecision, minPrecision) {
...
GridRefParserCI.prototype = new GridRefParser$1();
...
GridRefParserCI.prototype.parse = function (rawGridRef) {
  ...
  if (/[ABCDEFGHIJKLMNPQRSTUVWXYZ]$/.test(trimmedLocality)) {
    ...
    if (GridRefParser$1.quadrantOffsets.hasOwnProperty(trimmedLocality.substr(trimmedLocality.length - 2))) {
  ...
  if (/^(W[AV](?:\d\d){1,5})$/.test(trimmedLocality)) {
    if (enl = GridRefParserCI.gridref_string_to_e_n_l(trimmedLocality)) {
      ...
      this.gridCoords = new OSCIRef(enl.e, enl.n);
      ...
      if (this.length === 10000 && (tetradCode || this.quadrantCode)) {
        if (tetradCode) {
          ...
          this.gridCoords.x += GridRefParser$1.tetradOffsets[tetradCode][0];
          this.gridCoords.y += GridRefParser$1.tetradOffsets[tetradCode][1];
        } else {
          // quadrant
          ...
          this.gridCoords.x += GridRefParser$1.quadrantOffsets[this.quadrantCode][0];
          this.gridCoords.y += GridRefParser$1.quadrantOffsets[this.quadrantCode][1];
        }
      } else {
...
GridRefParserGB.prototype = new GridRefParser$1();
...
GridRefParserGB.prototype.parse_well_formed = function (rawGridRef) {
  ...
  if (rawGridRef.length >= 5 && /^[A-Z]/.test(rawGridRef.charAt(4))) {
    ...
    if (GridRefParser$1.quadrantOffsets.hasOwnProperty(rawGridRef.substr(rawGridRef.length - 2))) {
  ...
  if (this.tetradLetter || this.quadrantCode) {
    ...
    if (this.tetradLetter) {
      ...
      this.length = 2000; // 2km square
      this.gridCoords.x += GridRefParser$1.tetradOffsets[this.tetradLetter][0];
      this.gridCoords.y += GridRefParser$1.tetradOffsets[this.tetradLetter][1];
    } else {
      // quadrant
    ...
    if (this.length > 0) {
      ...
      if (this.length === 10000 && (tetradCode || this.quadrantCode)) {
        ...
        if (tetradCode) {
          ...
          this.tetrad = this.hectad + tetradCode;
          this.length = 2000; // 2km square
          this.gridCoords.x += GridRefParser$1.tetradKmOffsets[tetradCode][0];
          this.gridCoords.y += GridRefParser$1.tetradKmOffsets[tetradCode][1];
        } else {
          // quadrant
          this.preciseGridRef = trimmedLocality + this.quadrantCode;
          this.tetradLetter = '';
...
GridRefParserGB.prototype.parse_gr_string_without_tetrads = function (gridRef) {
  ...
  if ((matches = gridRef.match(/^(\d{2})\/((?:\d\d){1,5})$/)) !== null) {
    ...
    if (!GridRefParser$1.letterMapping.hasOwnProperty(gridRef.charAt(0)) || !GridRefParser$1.letterMapping.hasOwnProperty(gridRef.charAt(1))) {
      // invalid
      this.length = 0;
      this.gridCoords = null;
      return;
    ...
    var char1 = GridRefParser$1.letterMapping[gridRef.charAt(0)];
    var char2 = GridRefParser$1.letterMapping[gridRef.charAt(1)];
  ...
  switch (ref.length) {
    case 2:
      this.gridCoords = new OSRef(x + ref.charAt(0) * 10000, // use first digit of ref
    ...
    case 4:
      this.gridCoords = new OSRef(x + Math.floor(ref / 100) * 1000, y + ref % 100 * 1000);
      this.length = 1000; //1 km square
      break;

    case 6:
      this.gridCoords = new OSRef(x + Math.floor(ref / 1000) * 100, y + ref % 1000 * 100);
      this.length = 100; //100m square
      break;

    case 8:
      this.gridCoords = new OSRef(x + Math.floor(ref / 10000) * 10, y + ref % 10000 * 10);
      this.length = 10; //10m square
      break;

    case 10:
      this.gridCoords = new OSRef(x + Math.floor(ref / 100000), y + ref % 100000);
...
GridRefParserGB.prototype.parse_wellformed_gb_gr_string_no_tetrads = function (gridRef) {
  ...
  char1 = GridRefParser$1.letterMapping[gridRef.charAt(0)];
  char2 = GridRefParser$1.letterMapping[gridRef.charAt(1)];
  ref = gridRef.substr(2);
...
GridRefParserIE.prototype = new GridRefParser$1();
...
GridRefParserIE.prototype.parse_gr_string_without_tetrads = function (gridRef) {
  ...
  switch (ref.length) {
    case 2:
      this.gridCoords = new OSIRef(x + ref.charAt(0) * 10000, y + ref.charAt(1) * 10000);
      this.length = 10000; //10 km square
      this.hectad = char + ref;
      break;

    case 4:
      this.gridCoords = new OSIRef(x + Math.floor(ref / 100) * 1000, y + ref % 100 * 1000);
      this.length = 1000; //1 km square
      this.hectad = char + ref.charAt(0) + ref.charAt(2);
      break;

    case 6:
      this.gridCoords = new OSIRef(x + Math.floor(ref / 1000) * 100, y + ref % 1000 * 100);
      this.length = 100; // 100m square
      this.hectad = char + ref.charAt(0) + ref.charAt(3);
      break;

    case 8:
      this.gridCoords = new OSIRef(x + Math.floor(ref / 10000) * 10, y + ref % 10000 * 10);
      this.length = 10; //10m square
      this.hectad = char + ref.charAt(0) + ref.charAt(4);
      break;

    case 10:
      this.gridCoords = new OSIRef(x + Math.floor(ref / 100000), y + ref % 100000);
      this.length = 1; //1m square
 ...
 */
GridRefParser$1.factory = function (rawGridRef) {
  var parser;
...
var BIGU = {
  scriptVersions: {},
  GridRefParser: GridRefParser$1,
  GridRefParserCI: GridRefParserCI,
  GridRefParserGB: GridRefParserGB,
  GridRefParserIE: GridRefParserIE,
  LatLng: LatLng,
  CILatLng: CILatLng,
  IELatLng: IELatLng,
  MappingUtils: MappingUtils,
  NationalGridCoords: NationalGridCoords,
  OSCIRef: OSCIRef,
  OSGB36LatLng: OSGB36LatLng,
  OSIRef: OSIRef,
  OSRef: OSRef,
