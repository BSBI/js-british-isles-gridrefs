var GridRef = function () {
  ...
  GridRef.prototype.set_tetrad = function () {
    this.tetradLetter = GridRef.tetradLetters.substr((Math.floor(this.osRef.x % 10000 / 1000) >> 1) * 5 + (Math.floor(this.osRef.y % 10000 / 1000) >> 1), 1);
...
var LatLngCI = /*@__PURE__*/function () {
	/**
...
var GridCoordsCI = /*@__PURE__*/function () {
...
var GridRefCI = /*@__PURE__*/function () {
...
var GridRefGB = function () {
  ...
  */
	GridRefGB.prototype.parse_gr_string_without_tetrads = function (gridRef) {
		...
		switch (ref.length) {
			case 2:
				this.osRef = new GridCoordsGB(x + ref.charAt(0) * 10000, // use first digit of ref
			...
			case 4:
				this.osRef = new GridCoordsGB(x + Math.floor(ref / 100) * 1000, y + ref % 100 * 1000);
				this.length = 1000; //1 km square
				break;

			case 6:
				this.osRef = new GridCoordsGB(x + Math.floor(ref / 1000) * 100, y + ref % 1000 * 100);
				this.length = 100; //100m square
				break;

			case 8:
				this.osRef = new GridCoordsGB(x + Math.floor(ref / 10000) * 10, y + ref % 10000 * 10);
				this.length = 10; //10m square
				break;

			case 10:
				this.osRef = new GridCoordsGB(x + Math.floor(ref / 100000), y + ref % 100000);
...
var GridRefIE = function () {
  ...
  GridRefIE.prototype.parse_gr_string_without_tetrads = function (gridRef) {
    ...
    switch (ref.length) {
      case 2:
        this.osRef = new GridCoordsIE(x + ref.charAt(0) * 10000, y + ref.charAt(1) * 10000);
        this.length = 10000; //10 km square
        this.hectad = char + ref;
        break;

      case 4:
        this.osRef = new GridCoordsIE(x + Math.floor(ref / 100) * 1000, y + ref % 100 * 1000);
        this.length = 1000; //1 km square
        this.hectad = char + ref.charAt(0) + ref.charAt(2);
        break;

      case 6:
        this.osRef = new GridCoordsIE(x + Math.floor(ref / 1000) * 100, y + ref % 1000 * 100);
        this.length = 100; // 100m square
        this.hectad = char + ref.charAt(0) + ref.charAt(3);
        break;

      case 8:
        this.osRef = new GridCoordsIE(x + Math.floor(ref / 10000) * 10, y + ref % 10000 * 10);
        this.length = 10; //10m square
        this.hectad = char + ref.charAt(0) + ref.charAt(4);
        break;

      case 10:
        this.osRef = new GridCoordsIE(x + Math.floor(ref / 100000), y + ref % 100000);
        this.length = 1; //1m square
